...
function buildOperationalSection(
  ...
  const intro =
    'Operational responsibility for the ISMS is assigned at the level of individual artifacts. Dilligent records a named owner or assignee for every policy, control, risk, evidence task, and vendor; this constitutes the live, auditable responsibilities matrix.';
    ...
    return {
      ...
      bullets: ownership.map((row) => {
        const owners =
          row.owners.length > 0
            ? row.owners.join(', ')
            : 'assigned per item in Dilligent';
...
export function buildRolesSections(
  ...
  const sections: IsmsExportSection[] = [
    ...
    {
      heading: 'Relationship to Dilligent application-access roles',
      intro:
        'The Dilligent platform assigns application-access levels that determine access to the software only:',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        {
          text: 'The authority to assign tasks and make decisions within the ISMS rests with top management, who assign it to the Security & Privacy Owner; the SPO in turn assigns operational ownership of individual controls, policies, risks, and tasks through Dilligent.',
        },
      ],
    },
    {
  ...
  sections.push({
    ...
    paragraphs: [
      {
        text: 'Responsibilities and authorities are made known throughout the organisation through policy acknowledgement at onboarding and annually, the per-item owner assignments recorded in Dilligent, and the leadership communications captured in management reviews.',
      },
    ],
  });
