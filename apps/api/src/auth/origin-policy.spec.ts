...
describe('isStaticTrustedOrigin', () => {
  ...
  it('trusts HTTPS subdomains of the wildcard domains', () => {
    expect(isStaticTrustedOrigin('https://anything.withpickle.dev')).toBe(true);
    expect(isStaticTrustedOrigin('https://anything.staging.withpickle.dev')).toBe(true);
  ...
  it('does not extend the wildcard match to plain HTTP', () => {
    expect(isStaticTrustedOrigin('http://anything.withpickle.dev')).toBe(false);
    expect(isStaticTrustedOrigin('http://anything.staging.withpickle.dev')).toBe(false);
  ...
  it('rejects unrelated and malformed origins', () => {
    expect(isStaticTrustedOrigin('https://withpickle.dev.untrusted.example')).toBe(false);
