# browserbase-execution Specification

## Purpose

How a browser evidence step decides what it is allowed to log in as, and what
that decision implies for the Browserbase session it runs in.

A step runs in one of two auth modes. `saved_session` reuses an organization's
stored browser auth profile — its cookie context, its login, its health status.
`public` runs a page that needs no login at all, in a throwaway non-persistent
session that never touches a profile.

The requirements below were established by the `add-public-browser-evidence`
change, which introduced `public` mode. They cover only the auth-mode boundary;
navigation, screenshot capture, upload, and evaluation are specified elsewhere
and behave the same in both modes.
