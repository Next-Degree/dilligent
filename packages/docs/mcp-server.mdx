---
title: 'MCP Server'
description: 'Drive your Dilligent compliance program from any AI assistant — Claude Desktop, Cursor, ChatGPT, Codex, Gemini, VS Code, Windsurf — using just your Dilligent API key.'
---

The Dilligent MCP server lets you and your team operate your entire compliance program from inside any modern AI assistant. Ask in plain language; the AI uses real Dilligent tools to read evidence, manage tasks, generate questionnaire answers, connect cloud accounts, and more — all under your existing API key and permissions.

> Built on the [Model Context Protocol](https://modelcontextprotocol.io/) — an open standard that lets AI clients talk to external tools. The Dilligent MCP server publishes hundreds of tools spanning evidence, policies, integrations, frameworks, cloud security, vendors, risks, people, and tasks.
...
1. Sign in to [Dilligent](https://app.trycomp.ai).
...
<Card title="Download for Claude Desktop" icon="download" href="https://api.trycomp.ai/mcp/download/claude-desktop">
  Always serves the latest Dilligent MCP bundle (`.mcpb`) — no version hunting.
</Card>
...
3. Claude Desktop shows an install prompt — confirm, then paste your Dilligent API key in the extension settings.
4. Restart any open chats. Dilligent tools are now available.
...
Click the button above, confirm in Cursor, then paste your Dilligent API key when prompted.
...
Click the button above, confirm in VS Code, then paste your Dilligent API key when prompted.
...
Claude Code registers the server immediately — Dilligent tools become available in your next session.
...
Codex registers the server immediately — Dilligent tools become available in your next session.
...
That's it. The first time you start a chat in your AI client after install, the assistant will discover the Dilligent tools and start using them.
...
If the install succeeded, the AI will call Dilligent's `list-tasks` tool and return tasks from your organization. If you see no tool calls or an authentication error, jump to **Troubleshooting** below.

## Troubleshooting

<AccordionGroup>

<Accordion title="The AI says it doesn't have Dilligent tools available">
Most common causes:
...
- Confirm the key hasn't been revoked in **Dilligent → Settings → API Keys**.
...
1. In **Dilligent → Settings → API Keys**, copy your key again (or create a new one).
...
- The Dilligent API is briefly unavailable — try again in a moment or check status.
</Accordion>
...
The MCP server publishes tools across the Dilligent surface:
...
- **Your API key, your scope.** The MCP server runs locally on your machine and authenticates to the Dilligent API using the key you paste. It carries the same permissions you configured for that key — no escalation.
- **Multi-tenant safety.** Every request is scoped to the organization the API key belongs to. The AI cannot read or modify data belonging to other Dilligent organizations.
- **Secrets stay on your machine.** Your API key lives in your AI client's local config file. It is never sent to Dilligent, npm, or any third party except your own AI client's vendor.
- **Sensitive endpoints are hidden.** Endpoints that handle raw secrets (credential rotation, vault material) are deliberately excluded from the MCP tool surface. They remain available in the Dilligent web UI for human operators only.
- **All actions are logged.** Mutations performed through the MCP server appear in your Dilligent audit log marked with API-key authentication, so you can review who did what.
...
| Build a custom internal tool, dashboard, or one-off automation | **[Dilligent API](/api-reference/overview)** directly |
| Pipe Dilligent data into your data warehouse on a schedule | **API** directly |
| Onboard a new auditor or external reviewer to your Trust Center | **Dilligent web UI** |
...
No. The server runs locally on each user's machine via `npx`. Dilligent hosts nothing on your behalf, and you do not need to deploy or maintain any infrastructure.
</Accordion>

<Accordion title="Does the MCP server work offline?">
No. The server is a thin translator between your AI client and the Dilligent API at `https://api.trycomp.ai`. An internet connection is required.
</Accordion>

<Accordion title="Can multiple teammates use it at the same time?">
Yes. Each teammate installs the MCP server on their own machine with their own Dilligent API key. Their actions are independently authenticated and audit-logged.
</Accordion>

<Accordion title="What if my AI client doesn't support MCP yet?">
You can still use the [Dilligent API](/api-reference/overview) directly. The MCP server is an additional way to interact with the same endpoints — it doesn't replace anything.
</Accordion>

<Accordion title="How do new features show up?">
New tools and improvements ship with each Dilligent release. Because `npx -y` always pulls the latest published version, your AI client picks up new tools automatically the next time it starts the MCP server.
</Accordion>
...
- **Direct API reference:** [Dilligent API](/api-reference/overview)
