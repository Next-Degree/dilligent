...
- Sign in on a schedule, **including 2FA** — Dilligent generates the one-time code for you.
...
1. **Connections** — the vendor logins Dilligent signs in with (**Settings → Browser connections**).
2. **Browser evidence** — the automations that use those logins, created inside an evidence task.
3. **Schedule** — Dilligent signs in, captures the page, and re-signs in on its own when a session expires.
...
3. You have the vendor login you want Dilligent to use — ideally a **dedicated service account** rather than a personal one.
...
2. Dilligent reads the sign-in page and shows the **exact fields it asks for** — it detects whether the login uses an email, a username, or extra fields like a workspace.
3. Choose how Dilligent signs in: with a **stored login**, or via **single sign-on (SSO)**, where you finish at your identity provider.
4. Enter the login. Dilligent signs in for you in a live browser. If a step needs you — a 2FA prompt, an email code — you can **take over in the same browser** and hand it back.

<Note>
  Credentials are stored **encrypted in a secure vault**. Dilligent keeps only a reference to
  them — never the raw password in plain text.
</Note>

## Unattended 2FA

This is what makes scheduled runs truly hands-off. If your login uses an authenticator app, give Dilligent the **setup key** once, and it generates the rotating 6-digit codes itself — so scheduled runs never wait on your phone.
...
<Frame caption="The setup-key field, with per-vendor steps — 'How do I find this key?' — generated from the vendor's current help docs.">
  <img src="/images/browser-2fa-setup-key.png" alt="Entering the authenticator setup key, with per-vendor guidance" />
</Frame>
...
- **Not sure where to find it?** Select **"How do I find this key?"** next to the field. Dilligent shows the steps for your specific vendor, checked against its current help docs.
...
<Note>
  Dilligent never stores the rotating 6-digit code — only the setup key, encrypted — and generates a
...
<Frame caption="Testing an instruction — watch the AI drive a real browser live, then review the screenshot and verdict before saving.">
  <img src="/images/browser-test-run.png" alt="Live test run — the AI controlling a browser to capture evidence" />
</Frame>
...
- Dilligent signs in, navigates to the page you described, and captures a screenshot stamped with the **requirement, source URL, and capture time**. If you added a check, it records a **pass or fail** with a short reason.
- If the saved session has **expired**, Dilligent **re-signs in on its own** — using the stored login and a freshly generated 2FA code. You don't have to do anything.
...
<Frame caption="Every vendor login in one place — with health status, search, and per-connection actions.">
  <img src="/images/browser-connections.png" alt="The Browser connections settings page" />
</Frame>
...
<Accordion title="A connection says “Needs reconnect”" description="The saved session expired or a login step needs a human" icon="rotate">
  ...
  this every time a session expires, add the **authenticator setup key** so Dilligent can re-sign in
  on its own.
</Accordion>
...
<Accordion title="The steps for finding my authenticator key look generic" description="Vendor-specific vs. general guidance" icon="circle-question">
  For well-known vendors, Dilligent shows steps checked against their current help docs. For others it
