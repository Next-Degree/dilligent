...
description: 'Use the Dilligent API to automate SOC 2 evidence, policies, Trust Access, security questionnaires, tasks, and compliance workflows.'
---

The Dilligent API lets engineering, security, and compliance teams connect their internal systems to the compliance workflows they run in Dilligent.
...
> **Prefer to drive Dilligent from an AI assistant?** The same endpoints documented here are also available as tools in the [Dilligent MCP server](/mcp-server) — install it once in Claude Desktop, Cursor, ChatGPT, Codex, or any MCP-compatible client and ask in plain language.

## Authentication

Most organization automation uses the `X-API-Key` header. Create scoped API keys inside your Dilligent organization, keep them server-side, and grant only the permissions needed for the workflow you are building.
...
The reference covers Dilligent's product APIs across compliance operations, not just a single workflow. API-key backed endpoints are the integration contract for organization automation, while app session routes are kept out of the public reference.
...
The endpoint reference is generated from the NestJS OpenAPI specification used by the Dilligent API. Endpoint titles, descriptions, examples, visibility rules, and Mintlify metadata are maintained in the API source so generated pages stay aligned with product behavior as routes change.
